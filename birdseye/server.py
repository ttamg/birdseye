...
def file_view(session, path):
    ...
    all_funcs = sorted(
        session.query(Function.name, Function.type).filter_by(file=path).distinct()
    )
...
def latest_call(session, path, func_name):
    ...
    return jsonify(
        dict(
            id=call.id,
            url=url_for(call_view.__name__, call_id=call.id),
        )
    )
...
def base_call_view(session, call_id, template):
    ...
    func = call.function
    return render_template(
        template, short_path=basename(func.file), call=call, func=func
    )
...
def ipython_iframe_view(call_id):
    ...
    """
    return render_template(
        "ipython_iframe.html", container_id="1234", port=7777, call_id=call_id
    )
...
def calls_by_body_hash(session, body_hash):
    ...
    calls = [Call.basic_dict(withattrs(Call(), **row._asdict())) for row in query]
    ...
    return DecentJSONEncoder().encode(
        dict(calls=calls, ranges=ranges, loop_ranges=loop_ranges)
    )
...
def main(argv=sys.argv[1:]):
    ...
    parser = argparse.ArgumentParser(
        description="Bird's Eye: A graphical Python debugger"
    )
    parser.add_argument(
        "-p", "--port", help="HTTP port, default is 7777", default=7777, type=int
    )
    parser.add_argument(
        "--host", help="HTTP host, default is '127.0.0.1'", default=None
    )
